...
baltimoreCityPm25 <- pm25[
  pm25$fips == '24510', c('Emissions', 'type', 'year')
]

aux <- transform(
  baltimoreCityPm25, type.year=paste(type, year, sep='.')
)

totalPm25EmissionsByTypeAndYear <- tapply(
  aux$Emissions, aux$type.year, sum
)

typesAndYears <- strsplit(
  names(totalPm25EmissionsByTypeAndYear), '.', fixed=TRUE
)

typesAndYears <- matrix(
  unlist(typesAndYears), ncol=2, byrow=TRUE
)

types <- factor(
  typesAndYears[, 1], levels=c('POINT', 'NONPOINT', 'ON-ROAD', 'NON-ROAD')
)
...
totalPm25Emissions <- data.frame(
  tons=totalPm25EmissionsByTypeAndYear / (10 ** 3), type=types, year=years
)

tobj <- (
  ggplot(totalPm25Emissions, aes(x=year, y=tons, group=type, color=type)) +
  geom_point() +
  geom_line() +
  facet_grid(type ~ .) +
  ylab('PM2.5 emission (Kilotons)') +
  ggtitle('Baltimore City\'s PM2.5 emission') +
  theme_bw() +
  theme(legend.position='none', plot.title=element_text(face='bold'))
)

ggsave('plot3.png', tobj, width=4.5, height=4.5, dpi=100)
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
